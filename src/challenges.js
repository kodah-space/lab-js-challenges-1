...
function multiplyBy(numberArray, multiplier) {
  ...
  let newarray = [];
  numberArray.forEach((element) => newarray.push(element * multiplier));
  return newarray;
}
...
function filterOut(originalArray, toRemoveArray) {
  if (originalArray.length == 0) {
    ...
    for (let i = 0; i < toRemoveArray.length; i++) {
      while ((index = originalArray.indexOf(toRemoveArray[i])) !== -1) {
...
function uniquifyArray(full) {
  if (full.length == 0) {
    return null;
  } else {
    return full.filter((value, index) => full.indexOf(value) === index);
  }
}
